...
type SaveUnauthAppointmentnRequest struct {
	...
	LocationType string `json:"location_type" validate:"required" example:"at home"`
	Unit string `json:"unit" validate:"required" example:"14"`
	Apt string `json:"apt" validate:"required" example:"14"`
	AnimalSizeId int `json:"animal_size_id" validate:"required" example:"1"`
	...
	Amount  float32        `json:"amount" validate:"required" example:"500.00"`
	//unauth user email
	Email string `json:"email" validate:"required,email,max=255" example:"john.doe@example.com"`
	SpecialistId int `json:"specialist_id" validate:"required" example:"3"`
